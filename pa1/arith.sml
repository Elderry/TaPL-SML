...
fun pp t =
    case t
     of True => print "True"
      | False => print "False"
      | If (t1, t2, t3) => (print "If(" ; pp t1; print ", "; pp t2; print ", " ; pp t3; print ")")
      | Zero => print "Zero"
...
(* big-step evaluator *)
fun evalBig t =
	if isValue t then t else
	case t of
		If (t1, t2, t3) =>
		if evalBig t1 = True then
			if isValue (evalBig t2) then evalBig t2 else raise NoRule
		else
			if evalBig t1 = False then
				if isValue (evalBig t3) then evalBig t3 else raise NoRule
			else raise NoRule
		| Succ t1 => if isNumber (evalBig t1) then Succ (evalBig t1) else raise NoRule
		| Pred t1 => if evalBig t1 = Zero then Zero else
			(case evalBig t1 of
				Succ t2 => if isValue t2 then t2 else raise NoRule
				| _ => evalBig t1)
		| IsZero t1 => if evalBig t1 = Zero then True else
			(case evalBig t1 of
				Succ t2 => if isValue t2 then False else raise NoRule
				| _ => evalBig t1)
