...
fun occurs (x, t) =
    case t
     of Bool => false
      ...
      | Arrow (t1, t2) =>
        occurs (x, t1) orelse occurs (x, t2)
      | Pair (t1, t2) =>
        occurs (x, t1) orelse occurs (x, t2)
...
fun subst (x, ty, t) =
    List.map (fn (s1, s2) => (Type.subst(x, ty, s1), Type.subst (x, ty, s2))) t
...
fun typeCheck t = 
    let val (ty, c) = check (fn x => let val _ = print x
...
val t2 = Let ("f0", Abs("x0", Pair (Var "x0", 
                                    Var "x0"))
